...
def _phase_stats(storage: SupabaseStorage, total_stocks: int) -> tuple[list[dict], list[dict], set[str]]:
    summaries=[]; coverage_rows=[]; all_symbols:set[str]=set()
    # Same effective ranges as backfill: unavailable archive periods are never advertised.
    for phase, effective_start, effective_end in available_phases():
        start_date=effective_start.isoformat(); end_date=effective_end.isoformat(); stats=storage.history_monitoring_stats(start_date,end_date)
        if not stats: continue
        symbols={str(r.get("trade_code") or "").upper() for r in stats if r.get("trade_code")}; all_symbols.update(symbols)
        earliest=min((str(r.get("earliest_date")) for r in stats if r.get("earliest_date")),default=start_date); latest=max((str(r.get("latest_date")) for r in stats if r.get("latest_date")),default=end_date)
        ...
        summaries.append({"id":phase.id,"label":phase.label,"startDate":earliest,"endDate":latest,"requestedStartDate":phase.start.isoformat(),"effectiveStartDate":start_date,"effectiveEndDate":end_date,"archiveClipped":effective_start>phase.start,"symbolsWithData":count,"totalStocks":total_stocks,"remainingStocks":max(total_stocks-count,0),"rows":rows,"coveragePct":pct,"status":"complete" if total_stocks and count>=total_stocks else "in_progress"})
        for r in stats:
            symbol=str(r.get("trade_code") or "").upper()
            if symbol: coverage_rows.append({"sector":sector_for_symbol(symbol),"symbol":symbol,"earliestDate":str(r.get("earliest_date")) if r.get("earliest_date") else None,"latestDate":str(r.get("latest_date")) if r.get("latest_date") else None,"rows":_int(r.get("row_count")),"targetPeriod":phase.label,"coveragePct":None,"status":"data_available"})
...
def monitoring(storage: SupabaseStorage, interval_seconds: int = 120) -> dict:
    ...
    try: phases,coverage_rows,history_symbols=_phase_stats(storage,total_stocks); total_history_rows=storage.history_total_rows()
    except Exception as exc: phases,coverage_rows,history_symbols=[],[],set(); daily_available=False; daily_error=str(exc)
    ...
    return {"supabase":{"connected":True,"status":"connected"},"dataSave":{"status":save_status,"lastResult":run_status},"engine":{"status":engine_status,"currentSector":None,"currentStocks":active_symbols if run_status=="running" else [],"currentStockCount":len(active_symbols) if run_status=="running" else 0,"currentTask":f"Historical {active_phase}: {', '.join(active_symbols)}" if run_status=="running" and active_phase else "Waiting for next 3-stock historical batch","currentPhase":active_phase,"collectionMode":"historical-phase-batch","intervalSeconds":interval_seconds},"universe":{"totalSectors":len(sectors),"sectorsWithHistory":len(sectors_with_history),"remainingSectors":max(len(sectors)-len(sectors_with_history),0),"totalStocks":total_stocks,"stocksWithHistory":len(history_symbols),"remainingStocks":max(total_stocks-len(history_symbols),0),"historicalCoveragePct":round((len(history_symbols)/total_stocks)*100,2) if total_stocks else 0.0},"phases":phases,"lastRun":{"timestamp":last_at,"status":run_status,"rowsSaved":_int(latest_run.get("rows_collected")),"error":latest_run.get("error_message")},"nextRun":{"expectedAt":(last_dt+timedelta(seconds=interval_seconds)).isoformat() if last_dt else None,"intervalSeconds":interval_seconds,"note":"Internal scheduler timing is overridden by web.py runtime scheduler state."},"latestSnapshot":{"timestamp":max((s["updatedAt"] for s in stocks if s["updatedAt"]),default=None),"rows":total_stocks},"dailyHistory":{"available":daily_available,"error":daily_error,"rowsLoadedForMonitoring":0,"totalRows":total_history_rows,"aggregation":"database"},"coverageRows":coverage_rows}

def tracker(storage: SupabaseStorage) -> list[dict]: return [{"symbol":i["symbol"],"status":"completed","totalRecordsInserted":i["count"],"lastUpdated":i.get("lastUpdated") or ""} for i in storage.symbol_counts()]
def logs(storage: SupabaseStorage) -> list[dict]: return [{"time":r.get("updated_at") or r.get("started_at") or "","symbol":"MARKET","status":r.get("status", ""),"message":r.get("error_message") or f"Collector run via {r.get('trigger','cron')}","records":_int(r.get("rows_collected"))} for r in storage.recent_runs()]
